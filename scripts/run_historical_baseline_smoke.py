#!/usr/bin/env python3
"""Run a tiny non-model historical baseline smoke report."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Read-only historical baseline smoke report.")
...
def _print_text(report: dict, report_path: Path | None) -> None:
    print("PivotQuant historical baseline smoke")
    ...
    print("\n[mature_label_counts_by_horizon]")
    for horizon, count in report.get("mature_label_counts_by_horizon", {}).items():
        print(f"  {horizon}: {count}")

    print("\n[forward_return_distribution_by_horizon]")
    for horizon, stats in report.get("forward_return_distribution_by_horizon", {}).items():
        print(f"  {horizon}: count={stats.get('count')} mean={stats.get('mean')} median={stats.get('median')}")
...
def main() -> int:
    ...
    report_path = write_baseline_report(report)
