...
echo "[$(timestamp)] INFO train_artifacts config calib_days=${RETRAIN_RF_CALIB_DAYS} calib_mode=${RF_CALIB_MODE:-recent_days} time_decay=${RF_TIME_DECAY_ENABLED:-false} half_life=${RF_TIME_DECAY_HALF_LIFE_DAYS:-45} filter_unresolved=${RF_FILTER_UNRESOLVED_EVENTS:-true} ema_max=${RF_EMA_MAX_PRICE:-1000}" | tee -a "${LOG_DIR}/retrain.log"
