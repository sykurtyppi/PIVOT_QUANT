...
def run() -> int:
    ...
    try:
        ...
        if not db_backup.exists():
            raise FileNotFoundError(f"Missing DB backup in {snapshot}")
        if not models_archive.exists():
            raise FileNotFoundError(f"Missing models archive in {snapshot}")
        if not reports_archive.exists():
            raise FileNotFoundError(f"Missing reports archive in {snapshot}")
