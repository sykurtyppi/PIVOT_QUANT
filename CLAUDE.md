...
- **`scripts/audit_regime_health.py`** (Phase 1: detect dormancy) / **`scripts/audit_regime_health_attribution.py`** (Phase 2: attribute what changed) / **`scripts/audit_regime_data_quality.py`** (Phase 2B: data-quality of the shift) / **`scripts/audit_held_out_feasibility.py`** — read-only diagnostic scripts that mirror serving threshold semantics (manifest-first, artifact fallback) and report fixed chronological slices/buckets without picking favorable slices.
...
A real "model dormant" investigation requires running `scripts/audit_regime_health.py` (Phase 1 detector; `audit_regime_health_attribution.py` then attributes a confirmed dormancy) AND inspecting the manifest's `thresholds_meta` for guard reasons.
